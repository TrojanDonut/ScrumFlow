...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            # Temporarily commented out project creation for testing
            # authentication
            try:
                if not Project.objects.filter(name='Sample Project').exists():
                    ...
                    for user in User.objects.exclude(username='admin'):
                        ...
                        if user.username == 'product_owner':
                            role = ProjectMember.Role.PRODUCT_OWNER
                        elif user.username == 'scrum_master':
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            'Sample project created with members'
                        )
                    )
                else:
                    self.stdout.write(
            ...
            try:
                ...
                if not UserStory.objects.filter(
                    ...
                    self.stdout.write(
                        self.style.SUCCESS('Initial user story already exists')
                    )
            except Exception as e:
                self.stdout.write(
                    self.style.WARNING(
            ...
            try:
                ...
                tasks = [
                    ...
                    },
                    {
                        ...
                        'description': (
                            'This is the second initial task for testing.'
                        ),
                        'status': 'IN_PROGRESS',
                ...
                for task_data in tasks:
                    if not Task.objects.filter(
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(f'Task created: {task.title}')
                        )
                    else:
                        self.stdout.write(
                            self.style.SUCCESS(
                                f'Task {task_data["title"]} already exists'
                            )
                        )
            except Exception as e:
                self.stdout.write(
                    self.style.WARNING(
                        f'Error creating initial tasks: {str(e)}'
                    )
                )
